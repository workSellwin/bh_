...
)
{
	...
	for($i = 0; $i < $arParams["NUM_CATEGORIES"]; $i++)
	{
		...
		//new 16.10.2019
		if($USER->IsAdmin()) {
			if ($arResult["CATEGORIES"][$i]['TITLE'] == 'iblock_catalog') {

				$prodId = [];

				foreach ($arResult["CATEGORIES"][$i]["ITEMS"] as $searchItem) {

					if (!empty($searchItem['ITEM_ID'])) {
						$prodId[$searchItem['ITEM_ID']] = true;
					}
				}

				if (count($prodId) && Cmodule::IncludeModule('catalog')) {

					$res = \Bitrix\Catalog\ProductTable::getList([
						'filter' => ['ID' => array_keys($prodId), "!AVAILABLE" => 'Y'],
						'select' => ['ID', 'QUANTITY', "AVAILABLE"]
					]);

					if ($arReq = $res->fetchAll()) {
				...
				foreach ($arResult["CATEGORIES"][$i]["ITEMS"] as $index => $searchItem) {
					if (!empty($searchItem['ITEM_ID']) && !$prodId[$searchItem['ITEM_ID']]) {
						unset($arResult["CATEGORIES"][$i]["ITEMS"][$index]);
					}
				}
			}
		}
